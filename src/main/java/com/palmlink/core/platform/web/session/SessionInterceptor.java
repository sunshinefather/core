...
import com.palmlink.core.platform.web.cookie.CookieSpec;
import com.palmlink.core.platform.web.session.provider.LocalSessionProvider;
import com.palmlink.core.platform.web.session.provider.MemcachedSessionProvider;
import com.palmlink.core.platform.web.session.provider.SessionProvider;
import com.palmlink.core.platform.web.site.SiteHelper;
...
public class SessionInterceptor extends HandlerInterceptorAdapter {
    ...
    private Key<String> cookieSessionId = null;
    private Key<String> cookieSecureSessionId = null;
    ...
    public void initialize() {
        cookieSessionId = Key.stringKey(siteSettings.getSessionKeyPrefix() + "SessionId");
        cookieSecureSessionId = Key.stringKey(siteSettings.getSessionKeyPrefix() + "SecureSessionId");
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (!Boolean.TRUE.equals(initialized)) {
            loadSession(sessionContext, cookieSessionId);

            if (request.isSecure()) {
                secureSessionContext.underSecureRequest();
                loadSession(secureSessionContext, cookieSecureSessionId);
            }
    ...
    private void saveAllSessions(HttpServletRequest request) {
        saveSession(sessionContext, cookieSessionId, false);
        if (request.isSecure()) {
            saveSession(secureSessionContext, cookieSecureSessionId, true);
        }
    }
